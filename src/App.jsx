...
import {
  ...
  Check, Search, Printer, Home, Archive, Receipt, RefreshCw
} from "lucide-react";
...
function printReport({ sales, expenses, mo, yr }) {
  ...
  overlay.innerHTML = `
    ...
    <div style="margin-top:30px;font-size:10px;color:#999;border-top:1px solid #eee;padding-top:10px">Erstellt am ${new Date().toLocaleDateString("de-DE")} · DOF CLOUD Business Dashboard · dofclouds.de</div>
  `;
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        ...
        :filtered.map(s=>(
          <Card key={s.id} style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3}}><span style={{fontFamily:"Barlow Condensed",fontSize:14,fontWeight:700,color:C.txt}}>{s.productName.split(" ").slice(0,4).join(" ")} · Gr. {s.size}</span><Badge color={SC[s.status]}>{SL[s.status]}</Badge></div>
...
function AusgView({ expenses, setExpenses }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {expenses.map(e=>(
          <Card key={e.id} style={{display:"flex",alignItems:"center",gap:12}}><div style={{background:(CC[e.category]||C.muted)+"22",borderRadius:6,padding:7}}><DollarSign size={14} color={CC[e.category]||C.muted}/></div><div style={{flex:1}}><Badge color={CC[e.category]||C.muted}>{e.category}</Badge><div style={{fontFamily:"Barlow",fontSize:11,color:C.muted,marginTop:3}}>{e.date}{e.note?` · ${e.note}`:""}</div></div><div style={{fontFamily:"Barlow Condensed",fontSize:17,fontWeight:700,color:C.red}}>{fmt(e.amount)}</div><Btn variant="danger" sm onClick={()=>del(e.id)}><Trash2 size={11}/></Btn></Card>
        ))}
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <div style={{fontFamily:"Barlow Condensed",fontSize:13,color:C.muted,marginBottom:22}}>Monatsbericht {MON[mo]} {yr} · dofclouds.de</div>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gap:14,maxWidth:540}}>
        ...
        <Card><SH>Über DOF Cloud</SH><div style={{fontFamily:"Barlow",fontSize:12,color:C.muted,lineHeight:1.7}}><strong style={{color:C.txt,fontFamily:"Barlow Condensed",fontSize:14,letterSpacing:"0.5px"}}>DOF CLOUD Business Dashboard</strong><br/>Internes Management-Tool für dofclouds.de<br/>Daten werden in der Cloud gespeichert (Supabase) — überall verfügbar.</div></Card>
      </div>
    </div>
  );
}
...
function Sidebar({ active, setActive, lowN }) {
  return (
    <aside style={{width:206,minHeight:"100vh",background:C.panel,borderRight:`1px solid ${C.bdr}`,display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflowY:"auto"}}>
      <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${C.bdr}`}}>
        <div style={{fontFamily:"Bebas Neue",fontSize:24,letterSpacing:"4px",color:C.txt,lineHeight:1}}>DOF CLOUD</div>
      ...
      <div style={{padding:"10px 14px",borderTop:`1px solid ${C.bdr}`}}>
        <div style={{fontFamily:"Barlow",fontSize:9,color:C.dim}}>dofclouds.de</div>
...
export default function App() {
  ...
  const vp    = { products, setProducts, sales, setSales, expenses, setExpenses, customers, setCustomers, settings, setSettings };

  const VIEWS = {
    dashboard:<DashView  {...vp}/>, products:<ProdView  {...vp}/>, inventory:<LagerView {...vp}/>,
    sale:<SaleView {...vp}/>, orders:<OrdersView {...vp}/>, expenses:<AusgView  {...vp}/>,
  ...
  if (!ready) return (
    <div style={{background:C.bg,height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <div style={{fontFamily:"Bebas Neue",fontSize:32,letterSpacing:"6px",color:C.txt}}>DOF CLOUD</div>
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",color:C.txt}}>
      <Sidebar active={active} setActive={setActive} lowN={lowN}/>
      <main style={{flex:1,overflowY:"auto",padding:"26px 28px",minWidth:0}}>
        {VIEWS[active]}
      </main>
    </div>
  );
